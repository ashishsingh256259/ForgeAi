...
function getStrength(pw) {
  ...
  if (/[A-Z]/.test(pw)) return 3;
  if (/[0-9!@#$]/.test(pw)) return 4;
  return 2;
}

const STRENGTH_LABEL = ["", "Weak", "Fair", "Good", "Strong"];
const STRENGTH_COLOR = ["", "#EF4444", "#F97316", "#F59E0B", "#10B981"];
...
function AuthPage({ onAuthSuccess }) {
  ...
  function validate() {
    ...
    if (isSignup) {
      ...
      if (USER_STORE[email.toLowerCase()]) return "Account already exists. Please log in.";
    } else {
      const s = USER_STORE[email.toLowerCase()];
      if (!s)           return "No account found. Please sign up first.";
      if (s.pw !== password) return "Incorrect password. Please try again.";
    }
    return null;
  }
  ...
  function handleSubmit(e) {
    ...
    setTimeout(() => {
      setLoading(false);
      if (isSignup) {
        USER_STORE[email.toLowerCase()] = { name: name.trim(), email: email.toLowerCase(), pw: password };
        setSuccess("Account created! Redirecting...");
        setTimeout(() => onAuthSuccess({ name: name.trim(), email: email.toLowerCase() }), 900);
      } else {
        const s = USER_STORE[email.toLowerCase()];
        setSuccess(`Welcome back, ${s.name}!`);
        setTimeout(() => onAuthSuccess({ name: s.name, email: s.email }), 800);
      }
    }, 800);
  }

  return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#080B14", padding:"24px 16px", fontFamily:"'DM Sans', sans-serif" }}>
      <div style={{ width:"100%", maxWidth:440, background:"#141B2D", border:"1px solid rgba(255,255,255,0.08)", borderRadius:20, padding:"32px 28px" }}>

        {/* Logo */}
        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:28 }}>
          <div style={{ width:36, height:36, borderRadius:10, background:"linear-gradient(135deg,#F59E0B,#D97706)", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700, fontSize:18, color:"#000" }}>F</div>
          <span style={{ fontSize:20, fontWeight:700, color:"#F8FAFC" }}>Forge</span>
        </div>

        {/* Tab bar */}
        <div style={{ display:"flex", background:"#0D1120", borderRadius:12, padding:4, gap:4, marginBottom:24 }}>
          {["signup","login"].map(t => (
            <button key={t} onClick={() => switchTab(t)} style={{ flex:1, padding:"10px", borderRadius:9, border:"none", cursor:"pointer", fontWeight:600, fontSize:14, transition:"all 0.2s", background: tab===t ? "#F59E0B" : "transparent", color: tab===t ? "#000" : "#64748B" }}>
        ...
        {/* Heading */}
        <h2 style={{ fontSize:24, fontWeight:700, color:"#F8FAFC", margin:"0 0 4px" }}>
          {isSignup ? "Create your account" : "Welcome back 👋"}
        </h2>
        <p style={{ fontSize:13, color:"#64748B", margin:"0 0 24px" }}>
          {isSignup ? "Join thousands building their dream career with Forge." : "Log in to continue your personalized journey."}
        </p>
        ...
        <form onSubmit={handleSubmit} noValidate style={{ display:"flex", flexDirection:"column", gap:14 }}>
          {isSignup && (
            <div>
              ...
              <input type="text" placeholder="e.g. Rahul Sharma" value={name} onChange={e=>setName(e.target.value)}
                style={{ width:"100%", padding:"13px 16px", borderRadius:12, background:"#0D1120", border:"1px solid rgba(255,255,255,0.1)", color:"#F8FAFC", fontSize:14, outline:"none", boxSizing:"border-box" }} />
            </div>
          )}
          ...
          <div>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
              ...
              {!isSignup && <button type="button" style={{ background:"none", border:"none", color:"#F59E0B", fontSize:12, cursor:"pointer" }}>Forgot password?</button>}
            </div>
            <div style={{ position:"relative" }}>
              <input type={showPw?"text":"password"} placeholder={isSignup?"Min. 6 characters":"Enter your password"} value={password} onChange={e=>setPassword(e.target.value)}
                style={{ width:"100%", padding:"13px 44px 13px 16px", borderRadius:12, background:"#0D1120", border:"1px solid rgba(255,255,255,0.1)", color:"#F8FAFC", fontSize:14, outline:"none", boxSizing:"border-box" }} />
            ...
            {isSignup && password.length > 0 && (
              <div style={{ display:"flex", alignItems:"center", gap:6, marginTop:7 }}>
                <div style={{ display:"flex", gap:3, flex:1 }}>
                  {[1,2,3,4].map(i => <div key={i} style={{ flex:1, height:3, borderRadius:2, background: i<=strength ? STRENGTH_COLOR[strength] : "rgba(255,255,255,0.1)", transition:"background 0.3s" }} />)}
                </div>
                <span style={{ fontSize:11, color: STRENGTH_COLOR[strength], minWidth:38 }}>{STRENGTH_LABEL[strength]}</span>
              </div>
            )}
          </div>
          ...
          {isSignup && (
            <div>
              ...
              <div style={{ position:"relative" }}>
                <input type={showPw?"text":"password"} placeholder="Re-enter your password" value={confirm} onChange={e=>setConfirm(e.target.value)}
                  style={{ width:"100%", padding:"13px 44px 13px 16px", borderRadius:12, background:"#0D1120", border:"1px solid rgba(255,255,255,0.1)", color:"#F8FAFC", fontSize:14, outline:"none", boxSizing:"border-box" }} />
                {confirm.length > 0 && <span style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", fontSize:16, color: confirm===password?"#10B981":"#EF4444" }}>{confirm===password?"✓":"✗"}</span>}
              </div>
            </div>
          )}

          {error   && <div style={{ padding:"11px 14px", borderRadius:10, background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.3)", color:"#FC8181", fontSize:13 }}>⚠️ {error}</div>}
          {success && <div style={{ padding:"11px 14px", borderRadius:10, background:"rgba(16,185,129,0.08)", border:"1px solid rgba(16,185,129,0.3)", color:"#6EE7B7", fontSize:13 }}>✅ {success}</div>}

          <button type="submit" disabled={loading} style={{ width:"100%", padding:"14px", borderRadius:12, background:"linear-gradient(135deg,#F59E0B,#D97706)", color:"#000", fontWeight:700, fontSize:15, border:"none", cursor:"pointer", opacity: loading?0.7:1, marginTop:4 }}>
            {loading ? (isSignup?"Creating Account...":"Logging In...") : (isSignup?"Create Account & Continue →":"Log In to Forge →")}
          </button>
        </form>
        ...
        <div style={{ display:"flex", gap:10 }}>
          {[["🔵","Google"],["⬛","GitHub"]].map(([ico,lbl]) => (
            <button key={lbl} type="button" style={{ flex:1, padding:"11px 8px", borderRadius:12, background:"transparent", border:"1px solid rgba(255,255,255,0.1)", color:"#94A3B8", fontSize:13, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
              <span>{ico}</span>{lbl}
            </button>
          ))}
        </div>
        ...
        <p style={{ textAlign:"center", marginTop:18, fontSize:13, color:"#475569" }}>
          ...
          <button type="button" onClick={()=>switchTab(isSignup?"login":"signup")} style={{ background:"none", border:"none", color:"#F59E0B", fontWeight:700, fontSize:13, cursor:"pointer" }}>
